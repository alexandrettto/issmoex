...
class ISSMOEX:
    ...
    def __init__(self, concurrent_n=10):
        ...
        coroutine = self.fetch_all_data(parameters=self.engines_list, url_func=url, show_progress=False, pages=False)
        markets = self.run_fetcher(coroutine)
        markets_names =[df['NAME'].to_list()for df in markets]

        self.engines_markets = dict(zip(self.engines_list,markets_names))
    ...
    async def fetch_single_page(self, url):
        """
        Fetches a single page of data from the given URL.
    ...
    def securities_market(self, engine: str, market: str,main_board = True):
        ...
        cols_datetime = ['NEXTCOUPON', 'MATDATE', 'PREVDATE', 'SETTLEDATE', 'OFFERDATE', 'BUYBACKDATE'] if market == 'bonds' else ['SETTLEDATE']
        markets[cols_datetime] = markets[cols_datetime].apply(pd.to_datetime, errors='coerce')
        return markets
    ...
    def index_components(self, index, date, show_progress=False): 
        """
        Fetches index components for the given index and date(s).
        ...
        coroutine = self.fetch_all_data(parameters=dates, url_func=url_func, show_progress=show_progress, pages=False)
    ...
    def tables_description(self,engine: str, market: str): 
        ...
        url_func = lambda table: 'https://iss.moex.com/iss/engines/stock/markets/bonds.html?iss.only={}'.format(table)
        coroutine = self.fetch_all_data(parameters=tables, url_func=url_func, show_progress=False, pages=False)
        df_tables = self.run_fetcher(coroutine)
        result = dict(zip(tables,df_tables))
        return result 
    ...
    def iss_url(self,url,parameters = None, pages = False, show_progress = False):
        """
        Fetches data from a specified URL, handling pagination if necessary.
        ...
        if type(url) == str: 
            ...
            if parameters is None or parameters is not list:
                ...
                coroutine = self.fetch_all_data(parameters=parameters, url_func=url, show_progress=show_progress, pages=pages)
                df_tables = self.run_fetcher(coroutine)
    ...
    def engine_market_check(self, engine: str, market: str):
        ...
        if market not in self.engines_markets.get(engine, []):
